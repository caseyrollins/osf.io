...
class GuidMixinQuerySet(IncludeQuerySet):
    ...
    # In [251]: logs = NodeLog.objects.filter(node=OuterRef('pk')).order_by().values('node')
    # In [252]: count_logs = logs.annotate(cnt=Count('*')).values('cnt')
    # In [253]: qs = Node.objects.filter(title='test').annotate(log_count=Subquery(count_logs)).include('guids')

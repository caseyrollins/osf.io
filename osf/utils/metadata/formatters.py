...
from osf.utils.metadata import utils
...
class FileMetadataFormatter(object):
    ...
    def validate(self, record, metadata):
        return jsonschema.validate(metadata, record.schema)
...
class DataCiteMetadataFormatter(FileMetadataFormatter):
    ...
    def format_json(self, metadata_record):
        doc = {
            ...
            'creators': utils.datacite_format_creators_json(metadata_record.file.node.visible_contributors),
            'titles': [
                {
        ...
        if subjects:
            doc['subjects'] = utils.datacite_format_subjects_json(subjects)
